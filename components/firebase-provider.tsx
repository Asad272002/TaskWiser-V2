...
interface FirebaseContextType {
  ...
  createUserProfile: (profile: Omit<UserProfile, "id" | "createdAt" | "updatedAt">) => Promise<string>
...
export function FirebaseProvider({ children }: { children: React.ReactNode }) {
  ...
  const getUserProfile = async (address: string): Promise<UserProfile | null> => {
    if (!db) {
      console.error("Firestore is not initialized when trying to get user profile")
      return null
    }

    try {
      console.log(`Fetching user profile for address: ${address.toLowerCase()}`)
      const usersCollection = collection(db, "users")
      const q = query(usersCollection, where("address", "==", address.toLowerCase()))
      const querySnapshot = await getDocs(q)

      console.log(`User profile query returned ${querySnapshot.size} documents`)
  ...
  const getUserProfiles = async (): Promise<UserProfile[]> => {
    if (!db) {
      console.error("Firestore is not initialized when trying to get user profiles")
      return []
    }
  ...
  const createUserProfile = async (profile: Omit<UserProfile, "id" | "createdAt" | "updatedAt">): Promise<string> => {
    if (!db) {
      console.error("Firestore is not initialized when trying to create user profile")
      return ""
    }

    try {
      console.log(`Creating user profile for address: ${profile.address.toLowerCase()}`)
  ...
  const updateUserProfile = async (profileId: string, data: Partial<UserProfile>): Promise<void> => {
    if (!db) {
  ...
  const uploadProfilePicture = async (file: File, address: string): Promise<string> => {
    try {
      console.log(`Starting upload of profile picture for address: ${address.toLowerCase()} to IPFS`)
      ...
      console.log(`Profile picture uploaded successfully to IPFS. URL: ${ipfsUrl}`)
      return ipfsUrl
    } catch (error) {
      ...
      throw new Error(`Failed to upload profile picture to IPFS: ${errorMessage}`)
    }
  }
