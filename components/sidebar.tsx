...
import { Home, Kanban, User, ChevronLeft, ChevronRight, Folder } from "lucide-react"
...
export function Sidebar() {
  ...
  const navigation = [
    ...
    { name: "My Task Board", href: "/board", icon: Kanban },
    ...
    >
      ...
      <div className={cn("flex h-16 items-center px-4", collapsed ? "justify-center" : "justify-start")}>
        <Link href="/" className={cn("flex items-center gap-2", collapsed && "justify-center")}>
          ...
          {!collapsed && <span className="text-lg font-semibold">Task Wiser</span>}
        </Link>
      </div>
        ...
        {!collapsed && (
          <div className="flex-1 truncate">
            <div className="text-sm font-medium">
              {userProfile?.username || (account ? shortenAddress(account) : "Not connected")}
            </div>
          </div>
        )}
      </div>
